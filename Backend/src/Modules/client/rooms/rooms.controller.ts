...
export const getAllRooms = async (
  ...
  try {
    const rooms: IRoom[] = await Room.find()
      .populate('categoryId', 'name description basePrice capacity amenities images')
      .sort({ createdAt: -1 });
...
export const getRoomCategories = async (
  req: Request,
  res: Response<ApiResponse<{ categories: IRoomCategory[] }>>
): Promise<void> => {
  try {
...
export const getRoomImages = async (
  ...
  try {
    const room = await Room.findById(req.params.id).select('images');
