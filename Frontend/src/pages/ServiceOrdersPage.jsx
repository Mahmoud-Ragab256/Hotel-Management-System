...
const feedbackMeta = (type = "info") => {
  if (type === "success")
    return { title: "Success", icon: faCircleCheck, tone: "success" };
  if (type === "danger")
    return { title: "Error", icon: faCircleExclamation, tone: "danger" };
  if (type === "warning")
    return { title: "Attention", icon: faTriangleExclamation, tone: "warning" };
...
function FeedbackCard({ feedback, onClose }) {
  ...
  return (
    <Card
      className={`border-0 shadow-sm feedback-card feedback-card-${meta.tone}`}
    >
      <Card.Body className="p-3">
        <div className="d-flex align-items-start gap-3">
          <span
            className={`feedback-icon bg-${meta.tone}-subtle text-${meta.tone} rounded-circle d-inline-flex align-items-center justify-content-center flex-shrink-0`}
          >
...
function ServiceOrdersPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      showFeedback(
        "danger",
        `Could not load service orders: ${getApiErrorMessage(error)}`,
      );
      setOrders([]);
    } finally {
      setLoading(false);
  ...
  const adjustInvoiceForBooking = async (bookingId, delta) => {
    try {
      ...
      const invoice = invoices.find(
        (inv) => (inv.bookingId?._id || inv.bookingId) === bookingId,
      );
      if (!invoice) return;
  ...
  const handleCreateOrder = async (event) => {
    ...
    try {
      ...
      showFeedback(
        "success",
        "Service order created and added to the invoice.",
      );
      await loadAll();
    } catch (error) {
      showFeedback(
        "danger",
        `Could not create service order: ${getApiErrorMessage(error)}`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleAcceptOrder = async (order) => {
    ...
    try {
      ...
      showFeedback(
        "danger",
        `Could not accept order: ${getApiErrorMessage(error)}`,
      );
    } finally {
  ...
  const handleCompleteOrder = async (order) => {
    const assignedId =
      order.assignedEmployeeId?._id || order.assignedEmployeeId;
    if (
      assignedId &&
      currentEmployee?.id &&
      String(assignedId) !== String(currentEmployee.id)
    ) {
      showFeedback(
        "warning",
        "Only the employee who accepted this order can mark it complete.",
      );
      return;
    }
    ...
    try {
      ...
      showFeedback(
        "danger",
        `Could not complete order: ${getApiErrorMessage(error)}`,
      );
    } finally {
      setActionLoading((prev) => ({ ...prev, [orderId(order)]: null }));
    }
  ...
  const handleCancelOrder = async (event) => {
    ...
    try {
      ...
      if (price && bookingIdValue)
        await adjustInvoiceForBooking(bookingIdValue, -price);
      ...
      showFeedback(
        "danger",
        `Could not cancel order: ${getApiErrorMessage(error)}`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      showFeedback(
        "danger",
        `Could not delete order: ${getApiErrorMessage(error)}`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const selectedService = createForm.serviceId
    ? serviceById[createForm.serviceId]
    : null;
    ...
    <div className="d-flex flex-column gap-4">
              ...
              <div className="d-flex align-items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted mb-0">
                    Create, track, and manage guest service requests.
                  </p>
                </div>
              </div>
            </Col>
      ...
      <Card className="border-0 shadow-sm">
        <Card.Header className="bg-white border-0 p-4 pb-0">
          <Row className="g-3 align-items-center">
            <Col lg={5}>
              ...
              <p className="text-muted mb-0">
                View, accept, complete, cancel, and delete orders.
              </p>
            </Col>
            <Col lg={7}>
              <Row className="g-2">
        ...
        <Card.Body className="p-4">
          <div className="table-responsive">
            <Table
              hover
              className="align-middle mb-0 text-center admin-table-centered"
            >
                      ...
                      <tr key={orderId(order)}>
                        <td className="text-start">
                          <div className="d-flex align-items-center gap-2">
                            <span
                              className="bg-primary-subtle text-primary rounded-2 d-inline-flex align-items-center justify-content-center"
                          ...
                          <ButtonGroup size="sm">
                            ...
                            {status === "InProgress" && (
                              <Button
                                ...
                                title={
                                  assignedId && !isMine
                                    ? "Only the assigned employee can complete this order"
                                    : "Mark as completed"
                                }
                              >
                                {busy === "complete" ? (
                  ...
                  {bookings.map((b) => {
                    ...
                    const guestLabel =
                      ...
                      (guest?.firstName
                        ...
                        : b._id);
                    const room =
                      b.roomId?.number || b.room?.number || b.roomNumber || "?";
                    return (
                      <option key={b._id} value={b._id}>
                        Room {room} — {guestLabel}
                      </option>
                    );
                  })}
                </Form.Select>
      ...
      <Modal show={cancelModal} onHide={() => setCancelModal(false)} centered>
        <Form onSubmit={handleCancelOrder}>
          <Modal.Header closeButton>
            <Modal.Title>Cancel Service Order</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            ...
            (() => {
              ...
              return (
                <div className="d-flex flex-column gap-2">
                  ...
                  <div>
                    <strong>Quantity:</strong> {selectedOrder.quantity ?? 1}
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Badge bg={statusVariant(selectedOrder.status)}>
                  ...
                  {selectedOrder.assignedEmployeeId && (
                    <div>
                      ...
                      {selectedOrder.assignedEmployeeId?.fullName ||
                        selectedOrder.assignedEmployeeId?.name ||
                        String(selectedOrder.assignedEmployeeId)}
                    </div>
                  )}
                  {selectedOrder.notes && (
                    <div>
      ...
      <Modal show={deleteModal} onHide={() => setDeleteModal(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Delete Service Order</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          Are you sure you want to delete this service order? This action cannot
          be undone.
        </Modal.Body>
        <Modal.Footer>
          <Button
